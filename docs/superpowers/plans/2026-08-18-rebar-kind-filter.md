...
**주의 — 이 서술은 2026-08-18 리뷰에서 뒤집혔다.** 원래는 「분류 없음」을 자식 없는 잎 하나로 두려 했으나, 그러면 **축 전환에서 체크가 증발한다**: 부위축에서 개별 잎이던 미분류 철근이 종류축에서 잎 하나로 뭉쳐 `leafValues` 집합이 두 축에서 달라지기 때문이다(실측: 부위축 47잎 / 종류축 46잎, 전부 체크해도 45개만 보임). as-built 모델은 이름이 전부 레거시라 `kind` 가 전무하므로 이 경우가 실제로 흔하다.

→ 「분류 없음」은 **가지**로 만든다. 미분류 경로마다 정규화 키(`normalizePrimPath(stripComponentIndex(p))`)로 개별 잎을 매달아 두 축의 잎 신원을 같게 유지하고, 조상에도 경로를 쌓아 `count` 가 살아 있게 한다. 사용자 동작은 그대로다 — `RebarTreeRow` 가 `leafValues(node)` 를 토글하므로 「분류 없음」 부모를 클릭하면 여전히 미분류 철근이 전부 꺼지고, 개별 제어가 **추가**될 뿐이다.

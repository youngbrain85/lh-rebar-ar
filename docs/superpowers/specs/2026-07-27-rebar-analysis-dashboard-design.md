...
| 결정 | 내용 | 근거 |
|---|---|---|
| 분석 위치 | **브라우저 내** (메인스레드, 순수 TS — 실측 <100ms) | 서버 추가 없이 Vercel 그대로 배포. 중심선 데이터는 작아서 클라이언트 연산으로 충분. Web Worker는 Turbopack 번들 미지원 확인으로 철회 |
...
모두 `lib/analysis/`의 순수 함수. 메인스레드에서 실행 (useAnalysis 훅이 진행 상태 관리).
...
- **우측 패널**:
  ...
  - (v1 미구현·백로그: 분류별 집계 표 UI, 방향/레이어 표시 토글, 스캔 목록 '분석 여부' 배지 — summary.byGroup은 계산·저장됨)

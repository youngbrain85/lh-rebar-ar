...
깊이는 **가지마다 다르다**. 균일하지 않다는 것이 이 체계의 핵심 성질이다.
...
- 저판은 수평 부재라 전면/배면이 성립하지 않는다 → 상부/하부.
- 저판의 방향은 수직/수평이 무의미하다 → 횡방향(주철근)/종방향(배력철근).
- 헌치는 사재라 면·방향 구분이 없다 → 부위 바로 아래에 번호.
...
```
["전벽철근","전면","수직철근"], no=1  →  전벽철근-전면-수직철근-01
```
...
```jsonc
{
  ...
  "rebars": [
    {
      "prim": "/RebarModel/Stem_Front_Vert_01",  // USDZ prim 절대 경로 — 조인 키
      "label": "전벽-전면-수직철근-01",            // 화면 표시명 (생략 시 §4.2 규칙으로 조립)
      "path": ["전벽철근", "전면", "수직철근"],     // 트리 경로 (잎 제외, 깊이 가변)
      "no": 1
    },
    {
...
**구조물이 여럿인 모델**: 1 사이드카 = 1 구조물이 원칙이다. 한 USDZ에 구조물이 여럿이면 `structure`는 대표명일 뿐이고 **구조물 구분은 `path[0]`에 넣는다** — 예: `["옹벽 A", "전벽철근", "전면", "수직철근"]`. R4의 명시적 예외다.
...
| 출처 | 실제 문자열 예 |
|---|---|
| 사이드카 `prim` | `/RebarModel/Stem_Front_Vert_01` |
...
| 토큰 | 표시명 |
|---|---|
| `Stem` / `Base` / `Haunch` | 전벽철근 / 저판철근 / 헌치철근 |
| `Front` / `Rear` | 전면 / 배면 |
| `Top` / `Bot` | 상부 / 하부 |

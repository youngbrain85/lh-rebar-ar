...
**단, 이 불변식은 공짜가 아니다 (2026-08-18 리뷰에서 드러남).** 분류표에 없는 이름은 종류축에서 갈 곳이 없어 「분류 없음」으로 모이는데, 그 노드를 **잎 하나**로 두면 부위축의 개별 잎들과 대응이 끊겨 불변식이 깨진다. 실측: 45가닥 + 레거시 이름 2개를 넣으면 부위축 47잎 / 종류축 46잎이 되고, 부위축에서 전부 체크한 집합을 종류축에 넣으면 45개만 보인다 — 2가닥이 조용히 사라진다.

as-built 생성기 모델은 이름이 전부 레거시(`TopV_`/`BotH_`)라 `kind`가 **전무**하다. 그러면 종류축 트리가 「분류 없음」 하나가 되고 철근이 통째로 숨는다. 드문 경우가 아니다.

→ 「분류 없음」은 **가지**여야 한다. 미분류 경로마다 같은 정규화 키로 개별 잎을 매달면 두 축의 잎 집합이 실제로 같아진다. 사용자 동작은 바뀌지 않는다 — 부모를 클릭하면 서브트리 잎 전체가 토글되므로 "미분류 전부 끄기"는 그대로고, 개별 제어가 추가될 뿐이다.
